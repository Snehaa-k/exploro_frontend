...
const stripePromise = loadStripe(
  "pk_test_51PxSrYJiQplpQ67p3kTF3bBrVzigdJZPFHmLiY67aVcv66vff1cTyW4g9NCtjn1ZCbiMVi23UmvSyflOGJiiejIF005mqPKxuk",
);
...
const TripCard = ({ tripId }) => {
  ...
  const handleWalletPaymentClick = async () => {
    if (!wallet) {
      Swal.fire(
        "Error",
        "No wallet found! You cannot use wallet payment.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        Swal.fire(
          "Success",
          "Payment completed successfully using your wallet!",
          "success",
        );
        navigate("/success");
      } else {
      ...
      Swal.fire(
        "Error",
        "An error occurred while processing wallet payment.",
        "error",
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      <CardContent>
        <Typography
          variant="h6"
          style={{ color: is_booked ? "red" : "inherit" }}
        >
