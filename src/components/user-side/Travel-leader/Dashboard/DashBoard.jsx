...
const DashBoard = () => {
  ...
  const handleCreateGroup = async (tripId) => {
    ...
    const groupMembers = trip.booked_customers.map((customer) => ({
      user: customer.user,
    }));
  ...
  const completedTrips = trips.filter(
    (trip) => trip.is_completed === "completed",
  ).length;
  ...
  return (
    <Grid container direction="column" alignItems="center">
      <Grid item>
        <h4 style={{ marginTop: "-20px" }}>Travel Leader Dashboard</h4>
      </Grid>
      <Grid
        item
              ...
              {paginatedTrips?.map((trip) => (
                <React.Fragment key={trip.id}>
                  <TableRow>
                    ...
                    <TableCell>
                      {trip.booked_customers.length * trip.amount}
                    </TableCell>
