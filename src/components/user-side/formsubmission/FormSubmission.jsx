...
import { AppBar, Box, IconButton, Toolbar, Typography, Button, TextField, FormControl, Select, MenuItem, InputLabel } from '@mui/material';
...
export default function FormSubmission() {
  ...
  const validate=()=>{
    ...
    if (mobile.length <= 9) newErrors.mobile = 'Mobile Number must be  10 digits';
    if (!cvFile) {
      ...
      // dispatch(setUser(response))
        ...
        if (response) {
          Swal.fire({
            ...
            footer: '<strong>Note:</strong> You will receive an email regarding the acceptance or rejection of your form.',
            timer: 3000, 
  ...
  return (
    <div>
      ...
      <Box className="form-container" sx={{ textAlign: 'center' }} >
        <Typography variant="h6" sx={{ paddingTop: '40px' }}>Provide Valid Details</Typography>
      </Box>
      ...
      <Box
       ...
       onSubmit={handlesubmit}>
        <Box sx={{ display: 'flex', gap: 2, mb: 3, width: '100%', maxWidth: 600 }}>
          <TextField required id="first-name" label="First Name" value={firstname} fullWidth onChange={(e)=>setFirstname(e.target.value)}  error={Boolean(errors.firstname)} 
  helperText={errors.firstname}/>
          <TextField required id="last-name" label="Last Name" value={lastname}   onChange={(e)=>setLastname(e.target.value)}  fullWidth  error={Boolean(errors.lastname)} 
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, width: '100%', maxWidth: 600 }}>
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', mb: 2 }}>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flex: 1 }}>
              ...
              <Button component="label" variant="contained" onChange={(e) => handleFileChange(e, setCvFile)} startIcon={<CloudUploadIcon />} error={Boolean(errors.cvFile)} 
  helperText={errors.cvFile}>
            ...
            {errors.cvFile && <Typography color="error" variant="caption">{errors.cvFile}</Typography>}
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flex: 1 }}>
              <Typography>Upload Your ID Proof*</Typography>
              <Button component="label" variant="contained" onChange={(e) => handleFileChange(e, setIdFile)} startIcon={<CloudUploadIcon />} error={Boolean(errors.idFile)} 
  helperText={errors.idFile}>
            ...
            {errors.idFile && <Typography color="error" variant="caption">{errors.idFile}</Typography>}
          </Box>
        </Box>

        <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', maxWidth: 600, px: 2, mt: 2 }}  >
          <Button sx={{ bgcolor: 'black' }} variant="contained">BACK</Button>
          <Button sx={{ bgcolor: 'black' }} variant="contained" type="submit">SUBMIT</Button>
        </Box>
      </Box>
    </div>
  );
