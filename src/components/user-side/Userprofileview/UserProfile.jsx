...
const UserProfile = () => {
  ...
  const handleSubmitReport = () => {
    ...
    api
      .post(`/report/${id}/`, { reason: reportMessage })
      .then((response) => {
  ...
  const upcomingTrips = details.filter(
    (trip) =>
      moment(trip.end_date).isAfter(today) && trip.is_completed === "pending",
  );

  const completedTrips = details.filter(
    (trip) =>
      moment(trip.end_date).isBefore(today) &&
    ...
    <Box sx={{ bgcolor: "#f5f5f5", minHeight: "100vh" }}>
      <Box
        sx={{
          height: "150px",
          backgroundImage:
            "url(https://wallpapers.com/images/featured/solid-grey-background-ag4wgs25q8m35u89.jpg)",
      ...
      <Box sx={{ padding: "20px", display: "flex", justifyContent: "center" }}>
        <Grid container spacing={2}>
          {paginatedTrips.length > 0 ? (
                ...
                <Card
                  sx={{
                    ...
                    cursor:
                      ...
                      moment(trip.start_date).isAfter(today)
                        ? "pointer"
                        : "default",
                  }}
                  onClick={() => {
            ...
            <Typography
              variant="h6"
              sx={{ mt: 4, textAlign: "center", marginLeft: "700px" }}
            >
